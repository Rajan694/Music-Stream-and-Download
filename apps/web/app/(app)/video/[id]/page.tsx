...
export default function VideoPage({
  ...
  // Suggestions come from Piped only; when it is down the API answers 503, so
  // this query is allowed to fail quietly rather than break the page.
          ...
          <ul className="divide-y divide-zinc-200 dark:divide-zinc-800">
            {suggestions.map((item) => (
              <li key={item.id}>
                <Link
                  href={`/video/${item.id}`}
                  className="flex items-center gap-3 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 rounded-lg px-2 -mx-2"
                >

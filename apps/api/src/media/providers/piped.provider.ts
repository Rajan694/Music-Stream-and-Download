...
/** Raw Piped payloads. Deliberately loose — they never leave this file. */
type PipedPayload = Record<string, any>;

@Injectable()
export class PipedProvider implements MediaProvider {
  readonly name = 'piped';
  private readonly logger = new Logger(PipedProvider.name);

  constructor(private readonly config: MediaConfig) {}

  private get instances(): string[] {
    return [this.config.pipedApiUrl, ...this.config.pipedFallbackUrls].filter(
      Boolean,
    );
  }
  ...
  /**
   * Tries every configured instance, then retries the whole set with backoff.
   * Bounded by an overall deadline so a slow provider can never hold an HTTP
   * handler open for attempts x instances x timeout.
   */
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
      for (const instance of this.instances) {
        ...
        try {
          ...
          lastError = error;
          this.logger.debug(
            `piped ${instance} failed: ${(error as Error).message}`,
          );
        }
      }
  ...
  /** Piped reports item ids as relative URLs: `/watch?v=ID`, `/playlist?list=ID`. */
